  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ${isInvoiceMode ? 'bg-amber-50/50' : 'bg-gray-50'}`}>
      ...
      <div className={`sticky top-0 backdrop-blur-lg border-b px-4 lg:px-8 z-20 transition-colors duration-300 ${
        isInvoiceMode ? 'bg-amber-100/80 border-amber-200' : 'bg-white/80 border-gray-100'
      }`}>
        <div className="page-container flex items-center justify-between h-16">
          <h1 className={`text-xl font-bold ${isInvoiceMode ? 'text-amber-800' : 'text-gray-800'}`}>
            Trang chủ
          </h1>
        ...
        <div className="page-container space-y-6">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <Card className={`cursor-pointer shadow-card hover:shadow-card-hover transition-shadow ${isInvoiceMode ? 'border-amber-200 bg-white' : 'border-blue-200 bg-white'}`}>
              ...
              <p className={`text-xl font-bold ${isInvoiceMode ? 'text-amber-700' : 'text-blue-700'}`}>
              ...
              <div className={`mt-2 text-xs px-2 py-1 rounded-full inline-flex items-center gap-1 ${
                isInvoiceMode ? 'bg-amber-100 text-amber-600' : 'bg-blue-100 text-blue-600'
              }`}>
          ...
          <div className="grid lg:grid-cols-2 gap-6">
            ...
            <div className="space-y-6">
              ...
              <Card className={isInvoiceMode ? 'border-amber-200' : ''}>
                <h2 className={`font-semibold mb-4 text-lg ${isInvoiceMode ? 'text-amber-800' : 'text-gray-800'}`}>
                  Thao tác nhanh
                </h2>
            ...
            <Card className={`lg:row-span-2 ${isInvoiceMode ? 'border-amber-200' : ''}`}>
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  <h2 className={`font-semibold text-lg ${isInvoiceMode ? 'text-amber-800' : 'text-gray-800'}`}>
                    Đơn hàng gần đây
                  </h2>
                ...
                <button
                  ...
                  className={`text-sm font-medium flex items-center gap-1 ${
                    isInvoiceMode ? 'text-amber-600 hover:text-amber-700' : 'text-blue-600 hover:text-blue-700'
                  }`}
                >
                  Xem tất cả
              ...
              {currentOrders.length > 0 ? (
                  ...
                  {currentOrders.slice(0, 8).map(order => (
                    ...
                    >
                      ...
                      <div className="text-right">
                        <p className={`font-bold mb-1 ${isInvoiceMode ? 'text-amber-700' : 'text-gray-900'}`}>
                ...
                <div className="text-center py-12">
                  {isInvoiceMode ? (
                    <FileText size={48} className="mx-auto text-amber-300 mb-3" />
                  ) : (
                    <Package size={48} className="mx-auto text-gray-300 mb-3" />
                  )}
                  <p className={`font-medium ${isInvoiceMode ? 'text-amber-500' : 'text-gray-500'}`}>
