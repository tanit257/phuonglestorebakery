...
const CreateOrderPage = () => {
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ${isInvoiceMode ? 'bg-rose-50/50' : 'bg-gray-50'}`}>
      ...
      {currentCustomer && (
        <div className={`sticky top-0 z-10 shadow-sm ${isInvoiceMode ? 'bg-rose-50' : 'bg-white'}`}>
          <CustomerSelector
            selectedCustomer={currentCustomer}
            customers={[]}
            onSelect={() => {}}
            onDeselect={() => currentSetCustomer(null)}
            bgColor={isInvoiceMode ? 'rose' : 'violet'}
            displayMode="compact"
          />
        </div>
      )}
      ...
      <div className="px-4 lg:px-8 py-4 lg:py-6">
        <div className="page-container">
          ...
          <Card className={`mb-4 ${isInvoiceMode ? 'bg-rose-100 border-rose-300' : 'bg-blue-50 border-blue-200'}`}>
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className={`font-semibold ${isInvoiceMode ? 'text-rose-800' : 'text-blue-800'}`}>
                  {isInvoiceMode ? 'Chế độ Hóa đơn' : 'Chế độ Thực tế'}
                </p>
                <p className={`text-sm ${isInvoiceMode ? 'text-rose-600' : 'text-blue-600'}`}>
                  {isInvoiceMode
          ...
          {!currentCustomer && (
            <Card className={`mb-4 ${isInvoiceMode ? 'border-rose-200' : ''}`}>
              <CustomerSelector
            ...
            <div className="space-y-4">
              {currentCart.length > 0 ? (
                <Card className={isInvoiceMode ? 'border-rose-200' : ''}>
                  <CardTitle className={isInvoiceMode ? 'text-rose-800' : ''}>
                        ...
                        >
                          ...
                          <div className="mt-2 pt-2 border-t border-gray-200 flex flex-wrap items-center gap-2">
                            ...
                            <div className="flex-1 text-right">
                              ...
                              <span className={`font-semibold ${isInvoiceMode ? 'text-rose-700' : 'text-gray-800'}`}>
                ...
                <Card className={isInvoiceMode ? 'border-rose-200' : ''}>
                  ...
                  <div className="text-center py-12">
                    <ShoppingBag size={48} className={isInvoiceMode ? 'mx-auto text-rose-300 mb-3' : 'mx-auto text-gray-300 mb-3'} />
                    <p className={isInvoiceMode ? 'text-rose-400' : 'text-gray-400'}>
