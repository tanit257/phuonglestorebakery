...
import { Plus, Minus, Trash2, ShoppingBag, Eye, FileText, Package } from 'lucide-react';
...
const CreateOrderPage = () => {
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ${isInvoiceMode ? 'bg-violet-50/50' : 'bg-gray-50'}`}>
      ...
      {currentCustomer && (
        <div className={`sticky top-0 z-10 shadow-sm ${isInvoiceMode ? 'bg-violet-50' : 'bg-white'}`}>
          <CustomerSelector
            selectedCustomer={currentCustomer}
            customers={[]}
            onSelect={() => {}}
            onDeselect={() => currentSetCustomer(null)}
            bgColor={isInvoiceMode ? 'violet' : 'violet'}
            displayMode="compact"
          />
        </div>
      )}
      ...
      <div className="px-4 lg:px-8 py-4 lg:py-6">
        <div className="page-container">
          <div className="lg:grid lg:grid-cols-2 lg:gap-6 space-y-4 lg:space-y-0">
            {/* Mode indicator card */}
            <Card className={`lg:col-span-2 ${isInvoiceMode ? 'bg-violet-100 border-violet-300' : 'bg-blue-50 border-blue-200'}`}>
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className={`font-semibold ${isInvoiceMode ? 'text-violet-800' : 'text-blue-800'}`}>
                    {isInvoiceMode ? 'Chế độ Hóa đơn' : 'Chế độ Thực tế'}
                  </p>
                  <p className={`text-sm ${isInvoiceMode ? 'text-violet-600' : 'text-blue-600'}`}>
                    {isInvoiceMode
            ...
            {/* Customer Selection - only show when no customer selected */}
            {!currentCustomer && (
              <Card className={`lg:col-span-2 ${isInvoiceMode ? 'border-violet-200' : ''}`}>
                <CustomerSelector
            ...
            {/* Product Search - full width when customer selected */}
            <Card className={`${currentCustomer ? 'lg:col-span-2' : ''} ${isInvoiceMode ? 'border-violet-200' : ''}`}>
              <ProductSelector
                title="Thêm sản phẩm"
                ...
                showStock={true}
                plusButtonColor={isInvoiceMode ? 'violet' : 'violet'}
            ...
            {currentCart.length > 0 && (
              <Card className={`lg:col-span-2 ${isInvoiceMode ? 'border-violet-200' : ''}`}>
                <CardTitle className={isInvoiceMode ? 'text-violet-800' : ''}>
                      ...
                      >
                        ...
                        <div className="mt-2 pt-2 border-t border-gray-200 flex flex-wrap items-center gap-2">
                          ...
                          {!isInvoiceMode && (
                            <div className="flex items-center gap-1">
                              ...
                              <input
                                ...
                                className="w-16 px-2 py-1 text-sm border border-gray-200 rounded-lg focus:ring-1 focus:ring-violet-500 focus:outline-none"
                                placeholder="0"
                              />
                              <select
                                value={item.discountType}
                                onChange={(e) => updateCartItemDiscount(item.product_id, item.discount, e.target.value)}
                                className="px-2 py-1 text-sm border border-gray-200 rounded-lg focus:ring-1 focus:ring-violet-500 focus:outline-none bg-white"
                              >
                          ...
                          <div className="flex-1 text-right">
                            ...
                            <span className={`font-semibold ${isInvoiceMode ? 'text-violet-700' : 'text-gray-800'}`}>
                ...
                <div className="mt-4 pt-4 border-t border-gray-100">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <span className={`text-2xl font-bold ${isInvoiceMode ? 'text-violet-600' : 'text-violet-600'}`}>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <Button
                      ...
                      disabled={!currentCustomer || currentCart.length === 0}
                      icon={Eye}
                      variant="secondary"
                    >
                      Xem trước
                  ...
                  {currentCart.length === 0 && currentCustomer && (
                    <p className="text-xs text-center text-rose-500 mt-2">
            ...
            {currentCart.length === 0 && (
              <div className="text-center py-12 lg:col-span-2">
                <ShoppingBag size={48} className={isInvoiceMode ? 'mx-auto text-violet-300 mb-3' : 'mx-auto text-gray-300 mb-3'} />
                <p className={isInvoiceMode ? 'text-violet-400' : 'text-gray-400'}>
